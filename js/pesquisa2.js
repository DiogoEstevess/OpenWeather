function Obter_Cidade() {
  ...
  let weather = {
    ...
    fetchWeather: function (city) {
      fetch(
        "https://api.openweathermap.org/data/2.5/weather?q=" + cidade.value + "&units=imperial&appid=" + this.apiKey + "&lang=pt"
      )
        .then((response) => {
          if (!response.ok) {
  ...
  let forecast = {
    ...
    fetchForecast: function (city) {
      fetch(
        "http://api.openweathermap.org/data/2.5/forecast?q=" + cidade.value + "&units=imperial&appid=" + this.apiKey + "&lang=pt"
      )
    ...
    displayForecast: function (data) {
      for (i = 8, j = 2; i <= data.list.length; i += 8, j++) {
        ...
        if (i == 40) {
          temp.innerText = "Temperatura: " + data.list[i - 1].main.temp + "ºF";
        } else {
          temp.innerText = "Temperatura: " + data.list[i].main.temp + "ºF";
        }
...
function Localizacao() {
  navigator.geolocation.getCurrentPosition((success) => {
    ...
    fetch(
      "https://api.openweathermap.org/data/2.5/weather?lat=" + latitude + "&lon=" + longitude + "&units=imperial&appid=74d90f8a0dc103698bf1cb634e5dc227&lang=pt"
    )
      .then((response) => response.json())
